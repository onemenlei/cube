...
$.tmpl(lab_nav_menu, menus_Items).appendTo('#education_management_menu');
//菜单第一项反白选中，内容显示第一项
$($("#education_management_menu.indexItems_nav_ul li")[0]).addClass("selected");
/*侧菜单切换
$("#education_management_menu.indexItems_nav_ul").on("click", "li", function () {
...
*/
//education_management 教育局管理
var education_management = {
    //生成datatables
    create_table: function () {
       table = $('#datatable_education_management').DataTable({
    "data": this.data,
    ...
    "columnDefs": [
        ...
        },
        {
            ...
            "bSort": false,
            "render": function (data, type, row, meta) {
                var html = `<button class="default_btn_small" onclick="link_btn_details('${data.codCode}','education_user','education_article')"><i class="iconfont icon-guanli"></i>管理</button>
<button class="default_btn_small green_btn" onclick="education_management.editor_open('${data.codCode}')"><i class="iconfont icon-bianji"></i>修改</button>
<button class="default_btn_small yellow_btn stop_btn" onclick="education_management.stop_btn('${data.codCode}',${meta.row},event)"><i class="iconfont icon-tingyong"></i>停用</button>
<button class="default_btn_small white_btn hidden" onclick="education_management.start_btn('${data.codCode}',${meta.row},event)"><i class="iconfont icon-tingyong"></i>启用</button>
<button class="default_btn_small red_btn hidden" onclick="Men.del_list({id:${data.codCode}},${meta.row},'../biz/sch/student/delStudent')"><i class="iconfont icon-shanchu"></i>删除</button>`;
    ...
    newly_btn: function (){
            //校验
            $("#education_management").formValidator();
        var r = $("#education_management").validate().form();
        if (r) {
            ...
            education_management.school_reset();
	    	document.getElementById("education_management_signupForm").reset();	
        }
    },
    //编辑学校管理窗口 打开
    editor_open: function (code){
        var html = $("#education_management_template").html();
	        Men.new_window('修改教育局信息', html, '取消', this.editor_btn);
    ...
    // 教育局管理/教育局创建第一项反白选中，内容显示第一项
    school_reset: function(){
        $("#education_management_ul.label_top_menu_ul li").eq(0).addClass("selected").siblings().removeClass("selected");
        $("#education_management_ul.label_top_menu_ul+div").children().eq(0).show().siblings().hide();
    },
    //把省、市、区县数据绑定在select中
