...
var channel_one = {
    ...
    create_table: function () {
       table = $('#datatable_channel_one').DataTable({
    "data": this.data,
    ...
    "columnDefs": [
        ...
        },
        {
            ...
            "bSort": false,
            "render": function (data, type, row, meta) {
                var html = `<button class="default_btn_small" onclick="link_btn_details('${data.codCode}','channel_admin','channel_article')"><i class="iconfont icon-guanli"></i>管理</button>
<button class="default_btn_small green_btn" onclick="channel_one.editor_open('${data.codCode}')"><i class="iconfont icon-bianji"></i>修改</button>
<button class="default_btn_small yellow_btn stop_btn" onclick="channel_one.stop_btn('${data.codCode}',${meta.row},event)"><i class="iconfont icon-tingyong"></i>停用</button>
<button class="default_btn_small white_btn hidden" onclick="channel_one.start_btn('${data.codCode}',${meta.row},event)"><i class="iconfont icon-tingyong"></i>启用</button>
<button class="default_btn_small red_btn hidden" onclick="Men.del_list({id:${data.codCode}},${meta.row},'../biz/sch/student/delStudent')"><i class="iconfont icon-shanchu"></i>删除</button>`;
    ...
    newly_btn: function (){
            //校验
            $("#channel_one").formValidator();
        var r = $("#channel_one").validate().form();
        if (r) {
            ...
            channel_one.school_reset();
	    	        document.getElementById("channel_one_signupForm").reset();	
        }
    },
    //编辑学校管理窗口 打开
    editor_open: function (code){
        var html = $("#channel_one_template").html();
	        Men.new_window('修改渠道信息', html, '取消', this.editor_btn);
    ...
    // 渠道管理/渠道创建第一项反白选中，内容显示第一项
    school_reset: function(){
        $("#channel_one_ul.label_top_menu_ul li").eq(0).addClass("selected").siblings().removeClass("selected");
        $("#channel_one_ul.label_top_menu_ul+div").children().eq(0).show().siblings().hide();
    },
    //把省、市、区县数据绑定在select中
