...
var classs = {    
    ...
    select_school1: function(){
        var self=this;
        var res={code:0,data:[{code:1123,name:"教育局1"},{code:1124,name:"教育局2"},{code:1125,name:"教育局3"}]};
        var html="<option>请选择</option>";
        $.each(res.data, function(index,value){
            html+=`<option value="${value.code}">${value.name}</option>`;
        });
        $("#school1").html(html);
        $("#school1").off("change").on("change", function(){
            var code=$(this).val();
            $("#school2").show();
            self.select_school2(code);
        });
    },
    select_school2: function(code){ //code是一级传来的编号
        console.log("一级编号："+code);
        var self=this;
        var res={code:0,data:[{code:1123,name:"业务1"},{code:1124,name:"业务2"},{code:1125,name:"业务3"}]};
        var html="<option>请选择</option>";
        $.each(res.data, function(index,value){
            html+=`<option value="${value.code}">${value.name}</option>`;
        });
        $("#school2").html(html);
        $("#school2").off("change").on("change", function(){
            var code=$(this).val();
            $("#school3").show();
            self.select_school3(code);
        });
    },
    select_school3: function(code){ //code是二级传来的编号
        console.log("二编号："+code);
        var self=this;
        var res={code:0,data:[{code:1123,name:"学校1"},{code:1124,name:"学校2"},{code:1125,name:"学校3"}]};
        var html="<option>请选择</option>";
        $.each(res.data, function(index,value){
            html+=`<option value="${value.code}">${value.name}</option>`;
        });
        $("#school3").html(html);
        $("#school3").off("change").on("change", function(){
            console.log("三编号："+$(this).val());
            alert("三个code都拿到了，直接查询！");
        });
    },
};
