...
var ZHGL = {
    //账号管理数据
    zhgl_data: function(callback){
        var res={code:0,data:[
            {code:109384,username:"张三",alias:"张局长",phone:1354232224,role:"局长"},
            {code:109385,username:"李四",alias:"李局长",phone:135409887,role:"班长"},
            {code:109386,username:"王五",alias:"大刀",phone:136283764,role:"群众"}
        ]};
        callback(res);
    },
    ...
    create_table:function(data){
        zhgl_table = $('#datatable_zhgl').DataTable({
            "data": data,
            "bAutoWidth": false, //是否自适应宽度
            /*"ajax": {
                "url": "datatable1.json",
            ...
            "columnDefs":[
                ...
                "searchable":false,
                "render":function(data,type,row,meta){
                    var html=`<button class="default_btn_small green_btn" onclick="ZHGL.editor_open('${data.code}')"><i class="iconfont icon-bianji"></i>修改</button>
                    <button class="default_btn_small green_btn" onclick="ZHGL.resetPassword('${data.code})">重置密码</button>
                    <button class="default_btn_small red_btn" onclick="Men.del_list({id:${data.id}},${meta.row},'../biz/sch/school/delSchool')"><i class="iconfont icon-shanchu"></i>删除</button>`;
                    return html;
                }
            }],
            "language": {
    ...
    //生成账号表
    create_zhgl: function(){
        var self=this;
        self.zhgl_data(function(res){
            if(res.code==0)
    ...
    //新增账号内容
    newly_open: function(){
        var html=template('zhglTemplate',{datas:{}});
        Men.new_PIP("新增局端账号",html,"zhgl_newly","取消","新增",function(){            
            $("#zhgl").formValidator();//校验
    ...
    editor_open: function(code){
        this.zhgl_data(function(res){
            //要用ajax找后台数据. 下一条是本地数据,程序员删掉！
        var datas=Men.list_post(code,res.data,'code');
        /****ajax找后台数据 
        var data={code: code};    
        var datas=Men.post_data('../biz/sch/school/addSchool',data);    
        ****/
            var html=template('zhglTemplate',{datas});
            Men.new_PIP("修改局端账号",html,"zhgl_edit","取消","修改",function(){            
                $("#zhgl").formValidator();//校验
                var r = $("#zhgl").validate().form();
                if(!r){return false;}
                alert("弹出窗口数据存入后台")
    ...
    //批量导入账号
    import_zhgl_account: function(){
        var html=`<div class="col-12"><input type="file" name="file" class="dropify-fr" data-default-file="" mulpiple /></div>`;
        Men.new_PIP("批量导入账号",html,"import_zhgl_account","取消","导入",function(){
            alert($("input[type='file']").val());
        });        
        Men.dropify(); //初始化上传控件
    },
