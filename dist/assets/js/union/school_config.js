...
var config = {
    ...
    linkageSelect: function(){
        //用ajax取后台res
        var res={code:0,data:[
            {code:'110101',name:"第7组别",children:[{code:'001',name:"第1学校"},{code:'002',name:"第2学校"}]},
            {code:'110108',name:"第8组别",children:[{code:'003',name:"第3学校"},{code:'004',name:"第4学校"}]}
        ]};
        var html="";
        $.each(res.data,function(index,value){
            var childrenJson=JSON.stringify(value.children);
            html+=`<option value="${value.code}" data-children='${childrenJson}'>${value.name}</option>`;
        });
        $("#linkage1").html(html);
        $("#linkage1").off("change").on("change", function(){
            var children = $("#linkage1 option:selected").data("children");
            let html="";
