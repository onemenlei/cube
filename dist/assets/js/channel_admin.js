//这是渠道角色页面 用到其他地方，搜索admin. 换成其他名称
var admin = {
    //生成datatables
            ...
            "columnDefs": [
                {
                    ...
                    "render": function (data, type, row, meta) {
                        var html = "";
                        $.each(admin.school_table_data, function (index, value) {
		...
		},
                {
                    ...
                    "searchable": false,
                    "render": function (data, type, row, meta) {
                        var html = `<button class="default_btn_small green_btn" onclick="admin.editor_open('${data.channelRoleId}')"><i class="iconfont icon-bianji"></i>修改</button><button class="default_btn_small red_btn" onclick="Men.del_list({id:${data.channelRoleId}},${meta.row},'../biz/sch/student/delStudent')"><i class="iconfont icon-shanchu"></i>删除</button>`;
                        return html;
                    }
		}],
            "language": {
    ...
    newly_open: function () {
        ...
        Men.new_window('新建渠道角色', html, '取消', this.newly_btn);
    ...
    editor_open: function (code) {
        ...
        ****/
        ...
        $("#inputtext_04").val(datas.codCode).prop("disabled",true); //渠道编号
        this.bind_roleCode_table_data();//select绑定角色编号
    ...
    editor_btn: function () {
        ...
        if (r) {
            //从input中传值
	        	...
	        	datas.schCodes = admin.return_school_ID();
	        	// 保存修改到后台并判断正确与否        
