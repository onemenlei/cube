...
var methods = {
    ...
    $.fn.treegridData = function (options, param) {
      ...
      //当在表中选中checkbox时，它相对应的checkbox也选上
        target.on("click", "input[type='checkbox']", function(){
            if($(this).is(":checked"))
                {
                    var kid = $(this).attr("data-id");
                    var pid = $(this).attr("data-parentid");
                    var chkboxs = $("#tb").find("input[type='checkbox']");
                    $.each(chkboxs, function(index,value){                        
                        if($(this).attr("data-id") == pid)
                            $(this).prop("checked",true);
                        else if($(this).attr("data-parentid") == kid)
                            $(this).prop("checked",true);
                    });
                }
        });
        ...
        target.getChildNodes = function (data, parentNode, parentIndex, tbody) {
            $.each(data, function (i, item) {
                if (item[options.parentColumn] == parentNode[options.id]) {
                    var tr = $('<tr></tr>');
                    var nowParentIndex = (parentIndex + (j++) + '10A');
                        ...
                        if(column.field)
                            {
                                if(column.field.indexOf(".") != -1){
                                    var arr = column.field.split(".");
                                    td.text(item[arr[0]][arr[1]]);
                                }
                                else
                                {if(column.checkbox == '1')
                                    td.html(`<label><input type="checkbox" data-id="${item.menuCode}" data-parentid="${item.parentCode}" />${item[column.field]}</label>`);
                                else
                                    td.text(item[column.field]);}
                            }
                        else
                            {td.html(trees(item));td.addClass("center");}    
                            tr.append(td);
                    });
                    tbody.append(tr);
        ...
        target.addClass('table-treegrid');
        /*if (options.striped) {
